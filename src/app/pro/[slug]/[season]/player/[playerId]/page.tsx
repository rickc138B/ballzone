...
type PlayerData = {
  ...
  season: { id: string; label: string; slug: string }
...
export default function ProPlayerSeasonPage() {
  const { slug, season, playerId } = useParams() as { slug: string; season: string; playerId: string }
  ...
  useEffect(() => {
    fetch(`/api/pro/${slug}/${season}/player/${playerId}`)
      .then(r => r.json())
      .then(d => { setData(d); setLoading(false) })
  }, [slug, season, playerId])
  ...
  const { player, season_stats, career_high, game_log } = data

  return (
    <main className="min-h-dvh flex flex-col max-w-lg mx-auto px-5 pb-16">
      <div className="pt-6 mb-6">
        <Link href={`/pro/${slug}/${season}`} className="text-white/30 text-sm flex items-center gap-1.5 mb-6">
        ...
        {!season_stats ? (
          <p className="text-white/30 text-sm text-center py-8">No stats available for {data.season?.label}</p>
        ) : (
          <>
            {/* Career high */}
            {career_high && (
            ...
            <div className="card p-4 mb-4">
              <p className="text-white/30 text-xs uppercase tracking-wider mb-3">
                Season Averages · {season_stats.games_played} GP
              </p>
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  { label: 'PPG', value: season_stats.pts },
                  { label: 'RPG', value: season_stats.reb },
                  { label: 'APG', value: season_stats.ast },
                  { label: 'SPG', value: season_stats.stl },
                  { label: 'BPG', value: season_stats.blk },
                  { label: 'GP', value: season_stats.games_played },
                ].map(({ label, value }) => (
              ...
              <div className="grid grid-cols-3 gap-3 pt-3 border-t border-white/10">
                {[
                  { label: 'FG%', value: fmtPct(season_stats.fg_pct) },
                  { label: '3P%', value: fmtPct(season_stats.three_pct) },
                ].map(({ label, value }) => (
                  <div key={label} className="text-center">
            ...
            <div className="space-y-2">
              {game_log.map((g, i) => {
                ...
                return g.game_id ? (
                  <Link key={`${g.game_date}-${i}`} href={`/pro/${slug}/${season}/game/${g.game_id}`}>
                    {card}
                  </Link>
                ) : (
