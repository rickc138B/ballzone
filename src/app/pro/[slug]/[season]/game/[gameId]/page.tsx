...
function BoxScoreTable({ players, teamName, score, won, slug, season }: {
  players: PlayerGameStat[]; teamName: string; score: number; won: boolean; slug: string; season: string
}) {
  return (
    <div>
            ...
            {players.map(p => (
              <tr key={p.id}>
                <td className="px-4 py-2.5 whitespace-nowrap">
                  <Link href={`/pro/${slug}/${season}/player/${p.id}`} className="font-semibold text-white active:text-orange-400">{p.name}</Link>
                </td>
...
export default function GameDetailSeasonPage() {
  const { slug, season, gameId } = useParams() as { slug: string; season: string; gameId: string }
  ...
  useEffect(() => {
    ...
    const loadGame = () => fetch(`/api/pro/${slug}/${season}/game/${gameId}`)
      .then(r => r.json())
      .then(d => {
        setGame(d.game)
        setLoading(false)
        if (d.game && !d.game.status?.toLowerCase().includes('final')) {
          ...
          pollInterval = setInterval(async () => {
            try {
              const res = await fetch(`/api/pro/${slug}/${season}/game/${gameId}/live`)
              if (!res.ok) return
    ...
    loadGame()
    fetch(`/api/pro/${slug}/${season}/game/${gameId}/react`)
      .then(r => r.json()).then(setReactions)
    fetch(`/api/pro/${slug}/${season}/game/${gameId}/comments`)
      .then(r => r.json()).then(setComments)
    fetch(`/api/pro/${slug}/${season}/game/${gameId}/clips`)
  ...
  async function react(emoji: string) {
    ...
    await fetch(`/api/pro/${slug}/${season}/game/${gameId}/react`, {
      method: 'POST',
  ...
  async function submitComment() {
    ...
    const res = await fetch(`/api/pro/${slug}/${season}/game/${gameId}/comments`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function submitClip() {
    ...
    localStorage.setItem('ballzone:commentName', name)
    const res = await fetch(`/api/pro/${slug}/${season}/game/${gameId}/clips`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <main className="min-h-dvh flex flex-col max-w-2xl mx-auto pb-32">
      <div className="pt-6 px-4">
        <Link href={`/pro/${slug}/${season}`} className="text-white/30 text-sm flex items-center gap-1.5 active:text-white/60">
        ...
        {tab === 'box' && (
          <div className="space-y-4 mb-4">
            <div className="card overflow-hidden">
              <BoxScoreTable players={game.away_players} teamName={game.away_team.name} score={game.away_score} won={!homeWon} slug={slug} season={season} />
            </div>
            <div className="card overflow-hidden">
              <BoxScoreTable players={game.home_players} teamName={game.home_team.name} score={game.home_score} won={homeWon} slug={slug} season={season} />
            </div>
          </div>
        )}
