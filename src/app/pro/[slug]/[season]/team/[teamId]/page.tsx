...
export default function ProTeamSeasonPage() {
  const { slug, season, teamId } = useParams() as { slug: string; season: string; teamId: string }
  ...
  useEffect(() => {
    fetch(`/api/pro/${slug}/${season}/team/${teamId}`)
      .then(r => r.json())
      ...
      <div className="pt-6 mb-6">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-orange-400 text-sm font-medium mt-0.5">{data.season?.label}</p>
            <button
                  ...
                  {roster.map(p => (
                    <tr key={p.id}>
                      <td className="px-3 py-2.5">
                        <Link href={`/pro/${slug}/${season}/player/${p.id}`}
        ...
        {tab === 'games' && (
          <div className="space-y-2">
            ...
            {games.map(g => (
              <Link key={g.id} href={`/pro/${slug}/${season}/game/${g.id}`}>
