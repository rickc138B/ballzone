...
export async function GET(
  _req: NextRequest,
  { params }: { params: Promise<{ slug: string; season: string; teamId: string }> }
) {
  try {
    const { slug, season: seasonSlug, teamId } = await params
    ...
    const { data: season } = await seasonQuery.single()
    ...
    // Roster — players with season stats for this team, this season
    ...
    // Games involving this team, this season
    ...
    // Aggregate season standing, if we have one (e.g. Phase 1 totals)
    ...
    // Compute record from real per-game data
    ...
    return NextResponse.json({
      team,
      season,
      // prefer real computed record; fall back to aggregate season totals (e.g. Phase 1)
      record: gameSummaries.length > 0
        ? { ...record, gp: record.w + record.l }
        : teamSeason
          ? { w: teamSeason.wins, l: teamSeason.losses, pts_for: teamSeason.points_for, pts_against: teamSeason.points_against, gp: teamSeason.games_played }
          : { w: 0, l: 0, pts_for: 0, pts_against: 0, gp: 0 },
      roster: (roster ?? []).map(r => ({
