...
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ slug: string; season: string; playerId: string }> }
) {
  try {
    const { slug, season: seasonSlug, playerId } = await params
    ...
    const { data: seasonRow } = await seasonQuery.single()

    if (!seasonRow) return NextResponse.json({ error: 'Season not found' }, { status: 404 })
