...
async function verifyPin(supabase: any, leagueId: string, pin: string) {
  const { data } = await supabase.from('leagues').select('admin_pin_hash').eq('id', leagueId).single()
  if (!data) return false
  if (!data.admin_pin_hash) return true
  return bcrypt.compare(pin ?? '', data.admin_pin_hash)
}
...
export async function POST(
  ...
  try {
    ...
    const { game, home_team, away_team, admin_pin } = await req.json()
    const supabase = createServiceClient()

    const allowed = await verifyPin(supabase, leagueId, admin_pin)
    if (!allowed) return NextResponse.json({ error: 'Invalid admin PIN' }, { status: 403 })
    ...
    return NextResponse.json({ game_id: leagueGame.id, sequence_number: nextSeq, home_score: homeScore, away_score: awayScore, status })
  } catch (err) {
